// src/features/caixa/components/MovimentacaoCaixaModal.tsx
...
export function MovimentacaoCaixaModal({ tipo, onClose }: MovimentacaoCaixaModalProps) {
    ...
    const { mutateAsync, isPending } = useMutation({
        ...
        onSuccess: (_, variaveis) => {
            ...
            toast.success(`${tipo === 'SANGRIA' ? 'Sangria' : 'Reforço'} realizado com sucesso!`, {
                description: `Valor registrado: R$ ${variaveis.valor.toFixed(2)}`
            });
            onClose();
        },
        onError: (error: any) => {
            const msg = error.response?.data?.detail || "Erro ao processar a movimentação.";
